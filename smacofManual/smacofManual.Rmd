...
```{r localcode, echo = FALSE}
setwd("/Users/deleeuw/Desktop/smacofProject/smacofCode/smacofRR")
source("smacofRR.R")
setwd("/Users/deleeuw/Desktop/smacofProject/smacofCode/smacofManual")
```
...
```{r bsplines, echo = FALSE, cache = TRUE}
par(mfrow = c(1, 2))
...
```{r bernstein, echo = FALSE, cache = TRUE}
par(mfrow = c(1, 2))
makeBspline(x, 1, knots = NULL)
makeBspline(x, 3, knots = NULL)
```

```{r bsplinesord, echo = FALSE, cache = TRUE}
par(mfrow = c(1, 2))
...
```{r dcityresults, echo = FALSE, cache = TRUE}
source("results/hdcityResults.R")
```

```{r dcitykable, echo = FALSE, cache = TRUE}
x <- matrix(c(
0, 1, 0, 1, hdcity010$snew, hdcity010$itel,
0, 1, 0, 0, hdcity010o$snew, hdcity010o$itel,
0, 3, 0, 1, hdcity030$snew, hdcity030$itel, 
5, 3, 3, 1, hdcity533$snew, hdcity533$itel,
25, 0, 3, 1,hdcity2503$snew, hdcity2503$itel), 5, 6, byrow = TRUE)
knitr::kable(x, format = "pipe", digits = 10, col.names = c("ninner", "degree", "haveknots", "origin", "stress", "itel"), align = 'c',
caption = "Analyses of the dcity example")
```
...
```{r hdcitylinear, fig.align = "center", fig.cap = "dcity example, linear analysis", echo = FALSE, cache = TRUE}
par(mfrow = c(1,2))
smacofShepardPlot(hdcity010)
smacofShepardPlot(hdcity010o)
```
```{r hdcitycubic, fig.align = "center", fig.cap = "dcity example, cubic analysis", echo = FALSE, cache = TRUE}
par(mfrow = c(1,2))
smacofShepardPlot(hdcity030)
smacofShepardPlot(hdcity533)
```
```{r hdcityordinal, fig.align = "center", fig.cap = "dcity example, nonmetric analysis", echo = FALSE, cache = TRUE}
par(mfrow = c(1,2))
smacofShepardPlot(hdcity2503)
smacofShepardPlot(hdcity2503, fitlines = TRUE, knotlines = FALSE)
```
...
source("results/hekmanResults.R")
```

```{r ekmankable, echo = FALSE, cache = TRUE}
x <- matrix(c(
0, 3, 0, 1, hekman030$snew, hekman030$itel,
5, 3, 3, 1, hekman533$snew, hekman533$itel,
50, 0, 3, 1, hekman5003$snew, hekman5003$itel, 
50, 0, 2, 1, hekman5003o$snew, hekman5003o$itel), 4, 6, byrow = TRUE)
knitr::kable(x, format = "pipe", digits = 10, col.names = c("ninner", "degree", "haveknots", "origin", "stress", "itel"), align = 'c',
caption = "Analyses of the Ekman example")
```

```{r hekmancubic, fig.align = "center", fig.cap = "Ekman example, cubic analysis", echo = FALSE, cache = TRUE}
par(mfrow = c(1,2))
smacofShepardPlot(hekman030)
smacofShepardPlot(hekman533)
```
```{r hekmanordinal, fig.align = "center", fig.cap = "Ekman example, nonmetric analysis", echo = FALSE, cache = TRUE}
par(mfrow = c(1,2))
smacofShepardPlot(hekman5003, fitlines = TRUE, knotlines = FALSE)
smacofShepardPlot(hekman5003o, fitlines = FALSE, knotlines = TRUE)
```
