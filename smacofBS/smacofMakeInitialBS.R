...
smacofTorgersonBO <- function(thedata, p, itmax = 100, eps = 1e-10, 
                              verbose = TRUE) {
  indi <- thedata[, 1:2]
  delt <- thedata[, 3] ^ 2
  n <- max(indi)
  ...
  xold <- qr.Q(qr(smacofCenterBO(xold)))
  itel <- 1
  repeat {
    xaux <- smacofMatMult(indi, delt, xold)
    xbux <- outer(rep(1, n), dave) %*% xold
    xaux <- -0.5 * smacofCenterBO(xaux - xbux)
    xnew <- qr.Q(qr(xaux))
  ...
  lbd <- diag(crossprod(xaux, xnew))
  return(xnew %*% diag(sqrt(pmax(0, lbd))))
}
