...
int main(int argc, char **argv) {
    ...
    int n = 0, p = 0, init = 0, width = 0, precision = 0, relax = 0,
    ...
    /*
     * read row indices, column indices, and dissimilarities into a data
    ...
    strcat(strcpy(dataname, name), "DataU.txt");
    FILE *datafile = fopen(dataname, "r");
    int irowi, icoli;
    double deltai;
    for (int i = 0; i < m; i++) {
        fscanf(datafile, "%d %d %lf", &irowi, &icoli, &deltai);
    ...
    (void)smacofPrintVector(stdout, m, 8, 4, dhat);
    /*
    ...
    if (transform) {
        ...
        (void)smacofPrintAnyMatrix(stdout, m, nspan, 8, 4, basis);
        if (ordinal) {
    ...
    (void)smacofBSUEngine(n, p, m, irow, icol, xold, xnew, dmat, dhat, basis,
                          bcoef, itmax, fepsi, cepsi, verbose, relax, ditmax,
                          depsi, dverbose, nspan, ordinal, transform,
                          iterstring);
    // phew
    if (transform && checkkt) {
        (void)smacofCheckKuhnTuckerU(m, nspan, basis, bcoef, dhat, dmat);
    }
    double ***hessian = NULL;
    ...
    if (writefile) {
        ...
        (void)smacofWriteOutputFileU(output, n, p, m, irow, icol, width,
                                     precision, delta, dhat, xnew, dmat,
                                     iterstring);
        if (dohessian) {
        ...
        (void)smacofWriteShepardDataU(shepardput, m, irow, icol, width,
