...
int main(int argc, char **argv) {
    ...
    char *name = argv[1];
    char parname[SSIZE];
    int n = 0, p = 0, init = 0, width = 0, precision = 0, itmax = 0,
        haveweights = 0, fepsi = 0, verbose = 0, ditmax = 0, depsi = 0,
        dverbose = 0, jitmax = 0, jepsi = 0, jverbose = 0, kitmax = 0,
        kepsi = 0, kverbose = 0, degree = 0, ordinal = 0, haveknots = 0,
        ninner = 0, writefile = 0, labels = 0, dohessian = 0, dobmat = 0,
        checkkt = 0, anchor = 0, origin = 0;
    ...
    (void)smacofReadParameterFile(
        parameterfile, &n, &p, &init, &width, &precision, &itmax, &haveweights,
        &fepsi, &verbose, &ditmax, &depsi, &dverbose, &jitmax, &jepsi,
        &jverbose, &kitmax, &kepsi, &kverbose, &degree, &ordinal, &haveknots,
        &ninner, &writefile, &labels, &dohessian, &dobmat, &checkkt, &anchor,
        &origin);
    if (DEBUG) {
        (void)smacofWriteParameters(
            stdout, n, p, init, width, precision, itmax, haveweights, fepsi,
            verbose, ditmax, depsi, dverbose, jitmax, jepsi, jverbose, kitmax,
            kepsi, kverbose, degree, ordinal, haveknots, ninner, writefile,
            labels, dohessian, dobmat, checkkt, anchor, origin);
    }
    ...
    double **mdelta = (double **)calloc((size_t)n, sizeof(double *));
    (void)smacofMakeLowerTriangularMatrix(n, vdelta, mdelta);
    if (DEBUG) {
        printf("vdelta\n");
        (void)smacofPrintVector(stdout, m, width, precision, vdelta);
        printf("mdelta\n");
        (void)smacofPrintLowerTriangularMatrix(stdout, n, width, precision,
                                               mdelta);
    }
    double *vweight = NULL;
    double **mweight = NULL;
    ...
    if (haveweights) {
        ...
        if (DEBUG) {
            ...
            (void)smacofPrintLowerTriangularMatrix(stdout, n, width, precision,
                                                   mweight);
            printf("vinv");
            (void)smacofPrintSymmetricMatrix(stdout, n, width, precision,
                                             mvinv);
        }
    }
    ...
    if (DEBUG) {
        ...
        (void)smacofPrintRectangularMatrix(stdout, n, p, width, precision,
                                           mxold);
    }
    ...
    (void)smacofMakeBasis(m, ninner, order, degree, nspan, haveknots, vdhat,
                          name, vknots, vinner, mbasis);
    ...
    if (DEBUG) {
        printf("mbasis\n");
        (void)smacofPrintRectangularMatrix(stdout, m, nspan, width, precision,
                                           mbasis);
    }
    ...
    if (!haveweights) {
        (void)smacofBSUEngine(n, p, vxold, vxnew, mxold, mxnew, vdist, vdhat,
                              mdist, mdhat, mbasis, vcoef, itmax, fepsi,
                              verbose, ditmax, depsi, dverbose, kitmax, kepsi,
                              kverbose, nspan, ordinal, origin, iterstring);
    } else {
        (void)smacofBSWEngine(n, p, vweight, mvinv, vxold, vxnew, mxold, mxnew,
                              vdist, vdhat, mbasis, vcoef, itmax, fepsi,
                              verbose, ditmax, depsi, dverbose, nspan, ordinal,
                              origin, iterstring);
    }
    // phew
    /*
